...
type Voltage = 6 | 10 | 35 | 110;

const voltageColors: Record<Voltage, string> = {
  6: "#006600",
  10: "#6600cc",
  35: "#663300",
  110: "#0099ff",
...
export default function ElectroMapMVP() {
  ...
  const [lineVoltage, setLineVoltage] = useState<Voltage>(6);
    ...
    <Sheet open={!!selectedNode} onOpenChange={(o) => !o && setSelectedNode(null)}>
      <SheetContent>
        ...
        <div className="space-y-4">
          ...
          <div>Напряжение: {selectedNode.voltage}кВ</div>
          <div>
        ...
        {connecting && (
          <>
            ...
            <select
              className="ml-2 border rounded p-1"
              value={lineVoltage}
              onChange={(e) => setLineVoltage(Number(e.target.value) as Voltage)}
            >
              <option value={6}>6кВ</option>
              <option value={10}>10кВ</option>
              <option value={35}>35кВ</option>
              <option value={110}>110кВ</option>
            </select>
          </>
        )}
        <select
        ...
        >
          {filteredNodes.map((n) => (
            <Placemark
              ...
              properties={{ iconContent: nodeIcons[n.nodeType] }}
              options={{ preset: "islands#circleIcon", iconColor: voltageColors[n.voltage] }}
          ...
          {lines.map((l) => {
            ...
            return (
              <Polyline
                ...
                options={{ strokeColor: voltageColors[l.voltage], strokeWidth: 3 }}
              />
            );
          })}
        </Map>
      ...
      <Dialog open={nodeFormOpen} onOpenChange={setNodeFormOpen}>
        <DialogContent>
          ...
          >
            ...
            <div>
              <label className="block mb-1">Напряжение</label>
              <select
                className="w-full border rounded p-1"
